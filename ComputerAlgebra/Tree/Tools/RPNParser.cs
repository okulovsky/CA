...
namespace AIRLab.CA.Tools
{
    ...
    public class LongOperand : Operand, IArithmeticOperations, IComparisonOperations
    {
        ...
        public IOperand Pow(IOperand rhs)
        {
            if (!(rhs is LongOperand))
                throw new RPN_Exception("Argument invalid in LongOperand.Pow : rhs");
            LongOperand oprResult = new LongOperand("Result", Type.GetType("System.Int64"));
            oprResult.Value = (long)Math.Pow((long)this.Value , (long)((Operand)rhs).Value);
            return oprResult;
        }
    ...
    public class ArithmeticOperator : Operator
    {
        ...
        public override IOperand Eval(IOperand lhs, IOperand rhs)
        {
            ...
            switch (m_szOperator)
            {
                ...
                case "^":
                    return ((IArithmeticOperations)lhs).Pow(rhs);
            }
